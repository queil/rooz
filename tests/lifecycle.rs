...
async fn stop_and_start() {
    ...
    assert!(
        states.iter().all(|s| *s == ContainerSummaryStateEnum::RUNNING),
        "expected all containers running after rooz new, got {:?}", states
    );
    ...
    let states = env.workspace_container_states(&key).await;
    assert!(
        states.iter().all(|s| *s == ContainerSummaryStateEnum::EXITED),
        "expected all containers exited after rooz stop, got {:?}", states
    );
    ...
    assert!(
        states.iter().all(|s| *s == ContainerSummaryStateEnum::RUNNING),
        "expected all containers running after rooz start, got {:?}", states
    );
...
async fn restart() {
    ...
    assert!(
        states.iter().all(|s| *s == ContainerSummaryStateEnum::RUNNING),
        "expected all containers running after rooz restart, got {:?}", states
    );
